function [RouteMat,pathOut]=ShortestPath_Auto(image_input,TopPoint,MidPoint,J,inputPath,LeftOrRight)
K=4;
...
for cont=1:length(AllRoute)
    if AllRoute(cont)==1
        %         RouteMat(Pos{cont}(1),Pos{cont}(2),Pos{cont}(3))=...
        %             image_input(Pos{cont}(1),Pos{cont}(2),Pos{cont}(3));
...
for cont=1:size(image_input,3)
    ...
    hold off
    if LeftOrRight==0
        saveas(gcf,[inputPath,'\Left\Mask',num2str(cont),'.jpg'],'jpg')
    elseif LeftOrRight==1
        saveas(gcf,[inputPath,'\Right\Mask',num2str(cont),'.jpg'],'jpg')
    end
    clf
end
